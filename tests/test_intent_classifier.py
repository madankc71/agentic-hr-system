...
def classify_intent(state: HRState) -> HRState:
    ...
    # Eligibility must be checked FIRST (highest priority)
    if any(word in query for word in ["eligible", "eligibility", "exception", "contractor"]):
        state.intent = "eligibility_exceptions"

    elif any(word in query for word in ["policy", "working hours", "remote", "leave policy"]):
