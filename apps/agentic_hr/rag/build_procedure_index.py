from pathlib import Path
import faiss
import numpy as np

from apps.agentic_hr.rag.loaders.procedure_loader import load_procedure_documents
from apps.agentic_hr.rag.chunking.chunker import simple_chunk

from sentence_transformers import SentenceTransformer

INDEX_DIR = Path(__file__).parent / "indexes"
INDEX_DIR.mkdir(exist_ok=True)

model = SentenceTransformer("all-MiniLM-L6-v2")

def build_procedure_index():
    print("📂 Loading procedure docs...")
    docs = load_procedure_documents()

    texts = []
    metadata = []

    for doc in docs:
        chunks = simple_chunk(doc["text"])
        for chunk in chunks:
            texts.append(chunk)
            metadata.append({"filename": doc["filename"]})

    print(f"✂️ Created {len(texts)} chunks")

    embeddings = model.encode(texts)

    index = faiss.IndexFlatL2(embeddings.shape[1])
    index.add(np.array(embeddings))

    faiss.write_index(index, str(INDEX_DIR / "procedure.index"))
    
    np.save(INDEX_DIR / "procedure_texts.npy", np.array(texts, dtype=object))

    np.save(INDEX_DIR / "procedure_meta.npy", np.array(metadata, dtype=object))

    print("✅ Procedure index built")


if __name__ == "__main__":
    build_procedure_index()
